        ...
        <tbody class="bg-white">
          {
            ...
            <td class="px-4 py-3 text-xs border">
              {
                item.orderStatus === "Delivered" ? <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-sm"> {item.orderStatus} </span> : <span class="px-2 py-1 font-semibold leading-tight text-red-700 bg-red-100 rounded-sm"> {item.orderStatus} </span>
              }
            </td>
            <td class="px-4 py-3 text-sm border">&#8377; {item.totalPrice}</td>
            <td class="px-4 py-3 text-sm border"><Link to={`/ordersdetail/${item._id}`}><AspectRatioIcon /></Link></td>
          </tr>
            ))
          }
